<#
	...
	.\TraceHeap Start -EXE Name.exe[,Name2.exe...] [-Loop] [-CLR] [-JS]
	.\TraceHeap Start -ProcessID 1234 [-Loop] [-CLR] [-JS]
	.\TraceHeap Start -Lean [-Loop] [-CLR] [-JS]
	.\TraceHeap Stop [-WPA]
	.\TraceHeap View [-Path <path>\MSO-Trace-Heap.etl]
	.\TraceHeap Status
	.\TraceHeap Cancel
...
Param(
	...
	# Trace only VirtualAlloc(MEM_COMMIT)
...
# ===== CUSTOMIZE THIS =====
	...
	@{
		...
		ViewerConfig = ".\WPAP\Heap.wpaProfile"
...
# Main
	...
	if (!$Lean)
	{
		# TraceHeap.bat calls: PowerShell -file TraceHeap.ps1 %*
		# This doesn't interpret comma-separated array elements on input.
		# The alternative is: PowerShell -command TraceHeap.ps1 %*
	...
	Started
		...
		if (!(CheckOSVersion '6.2.0')) { Write-Warn "Warning: Stackwalks with symbols for VirtualAlloc traces are available starting with Windows 8.0`n" }
